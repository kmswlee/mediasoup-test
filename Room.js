  ...
  async _handleProtooRequest(peer, request, accept, reject) {
	...
	accept({kind});
      ...
      case "connectConsume": {
        ...
        const dtlsParameters = JSON.parse(request.data.dtlsParameters);
	console.log(dtlsParameters);
        if (peer.data?.consumeTransport) {
          try {
            await peer.data.consumeTransport.connect({ dtlsParameters:dtlsParameters });
          } catch (error) {
            console.log(error.message);
          }
        }
      ...
      case "consume": {
        //const rtpCapabilities = typeof request.data === 'object'?request.data.dtlsParameters:JSON.parse(request.data.dtlsParameters);
        const rtpCapabilities = JSON.parse(request.data.rtpCapabilities);
	console.log(this._producer.id);
        console.log(
